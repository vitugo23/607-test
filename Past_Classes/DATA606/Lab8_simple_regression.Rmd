...
**The dimensions of this dataset are 1458 cases and 123 columns**
...
**I would use a scatter plot, using a linear regression might be the best approach to display this relationship. **
...
**The plot displays a positively linear relationship. After calculating the correlation we know there is a strong correlation between the 2 variables, the form is linear, direction is positive, and the strength is 0.796 which it might be considered as strong **
...
**The smallest sum that I got was 1089.67 squares compare with the maximun of 2609.869**
...
**New model: y = hf_score(5.153687) + x = pf_expression_control( 0.349862)**

```{r}
nm <- lm(hf_score ~ pf_expression_control, data = hfi)
summary(nm)
```
...
**With the regression, we would expect a personal freedom score of 7.909651, and it seems to be underestimated by 0.1 **
...
**The residual plot shows that is randomly distributed, this will assure that we have a linear relationship between the two variables.**
...
**Based on both graphs, the conditions appear to be met. The histogram looks like a bell curve, and the line in the Q-Q plot look straight, meaning that the data is normally distributed.**
...
**It appears to meet the constant variability condition. The variance is constant in the positive and negative directions across the X-axis.**
...
```{r}
ggplot(hfi, aes(pf_rol_civil,pf_rol_criminal)) +
  geom_point() +
  labs(title = "Human freedom predicted by rol criminal")
```


**It looks to be a linear relationship between the two variables**
...
```{r}
nm1 <- lm(pf_rol_civil ~ pf_rol_criminal, data = hfi)
summary(nm1)
```
    

**The new model have a R-squared of 0.777 compared to 0.6342 in the original model. Since our previous model has a lower R_squared it is a not better predictor.**
...
```{r}
ggplot(hfi, aes(ef_regulation_labor_bargain, ef_score)) +
  geom_point() 
```
    
```{r}
hfi %>%
  summarise(cor(ef_regulation_labor_bargain, ef_score, use = "complete.obs"))
```
   
```{r}
nm2 <- lm(ef_score ~ ef_regulation_labor_bargain, data = hfi)
summary(nm2)
```
   
    
**After analyzing a couple of relationships, I found out that the regulation labor bargain does not affect the total economic freedom. **
