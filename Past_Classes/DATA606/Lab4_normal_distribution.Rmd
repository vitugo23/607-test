...
**According to the plot, the data does not appear to follow a nearly normal distribution, it does appears to has a slight skew to the right, while the center of the distribution seems to fall over to the right slightly.**
...
ggplot(data = dairy_queen, aes(sample = sim_norm)) + 
  geom_line(stat = "qq")
```
**All of the points do not fall on the line, this plot is similar but not the same from the probability plot for the real data.**
...
**The simulated data plots looks more regular than the data plots, that tell us that calories are nearly normal in the Dairy queen cal_fat data**
...
**McDonald’s plot is more curved compared to the simulated plots and does not appear to be a normal distribution.**
...
```{r}
ch_mean <- mean(dairy_queen$cholesterol)
ch_sd  <- sd(dairy_queen$cholesterol)
pnorm(q=154, mean= ch_mean, sd=ch_sd )
```
```{r}
chick_filet <- fastfood %>%
  filter(restaurant == "Chick Fil-A")

a_mean <- mean(chick_filet$protein)
a_sd <- sd(chick_filet$protein)

1 - pnorm(q = 30, mean = a_mean, sd = a_sd)

chick_filet %>% 
  filter(protein > 30) %>%
  summarise(percent = n() / nrow(chick_filet))
```
...
qqnorm(filet$sodium, main = "Chick Fil-A")
```
```{r}
queen <- fastfood %>%
  filter(restaurant == "Dairy Queen")

qqnorm(queen$sodium, main = "Dairy Queen")
```
```{r}
mac <- fastfood %>%
  filter(restaurant == "Mcdonalds")

qqnorm(mac$sodium, main = "McDonald's")
```
```{r}
sonic <- fastfood %>%
...
**based on the plots, Burger King has the distribution closest to normal, and subway the farest to it**
...
**Stepwise pattern changes in sodium distributions, because food chain restaurants offer a variety of products, and the sodium content across these different food groups varies drastically, changing the stepwise pattern **
...
**Based on the normal probability plot, the variable "total carbohydrates" is right skewed.**
```{r}
qqnorm(queen$total_carb, main = "Dairy Queen Carbs")
qqline(queen$total_carb)
```
```{r}
hist(queen$total_carb)
```
