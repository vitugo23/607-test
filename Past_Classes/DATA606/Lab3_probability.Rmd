...
**-A streak length of 1 means one hit is followed by one miss shot.**
**-There are three hits and six misses in a streak of 1 **
**-A streak length of 0 it will be 0 hits and 1 misses after the streak of 1 **
...
**The distribution of Kobe's streak length seems to be right skewed from the 2009 finals.**
**Based on the Histogram, Kobe's longest streak is 0 with a count of between 35-40**
**Kobe's longest streak was 4**
```{r}
kobe_streak <- calc_streak(kobe_basket$shot)
ggplot(data = kobe_streak, aes(x = length)) +
  geom_bar()
...
**In my simulation flipping the unfair coin 100, I got 16 heads and 84 tails    
```{r}
set.seed(122779)
coin_outcomes <- c("heads", "tails")
sample(coin_outcomes, size = 1, replace = TRUE)
...
#### the typical streak length of the simulated player is 0 with a count between 40-45. ####
#### the player's longest streak is 5. ####

```{r}
ggplot(data=sim_streak,aes(x=length), color= "darkblue")+
  geom_bar()
```
...
### If I were to run the simulation a second time I would expect the streak distribution to be almost the same because the probability of the number of hits is 45% we should expect to hit more misses than hits in those 133 total shots. ###
...
### Kobe's distribution of streak lengths seem to be fairly similar with the simulated shooter, they both tend to go on long streaks of length 0 and both are right-skewed, and the second highest would be a streak length of 1. I think that the hot hand modeldoes not fits Kobe’s shooting patterns because it appears to be independent shooter. ###
