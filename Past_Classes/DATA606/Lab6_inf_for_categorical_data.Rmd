...
#### The count is displayed with the method above: 4792 students did not text and drive. 4646 did not drive, etc.
...
#### The total of people who have texted while driving and never wore helmets is 464 which is a proportion of 7.66%.
...
#### The margin of error ranges between 6.5% and 7.7% for the estimate of the proportion of non-helmet wearers.
...
```
```{r}
unique(yrbss$physically_active_7d)
```
```{r}
forza <- yrbss |>
  filter(!is.na(physically_active_7d)) |>
  mutate(very_active = ifelse(physically_active_7d %in% c("1","2","3"), "yes", "no"))
...
```{r}
forza |>
  specify(response = very_active, success = "yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.95)
```
#### The 95% confidence interval for the probability physically active in 7 days is between  0.269 and 0.284.
...
#### We can se that the margin of error increases when the population proportion do the same.the margin of error reach the highest peak when the population reaches 50%..5 x .5 is the greatest that the numerator could ever be when calculating standard error.
...
#### It seems that the center is at 0.10, the spread at 0.017, and the shape is at a normal distribution bell curve.
...
#### When I change the P value to a smaller number, the data seems to maintain its normally distributed shape, however, it moves along the x-axis.

8.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

#### When I change N, the distribution of the data appears to maintain normality, and the center seems to be moving to the right.
...
workout <- yrbss |>
  ...
  mutate(daily = ifelse(strength_training_7d == "7", "yes", "no"))
...
```{r}
workout |>
 specify(response = daily, success = "yes") |>
 generate(reps = 1000, type = "bootstrap") |>
 calculate(stat = "prop") |>
 get_ci(level = 0.95)
```
...
```{r}
hours_sleep |>
 specify(response = ten_plus, success = "yes") |>
 generate(reps = 1000, type = "bootstrap") |>
 calculate(stat = "prop") |>
 get_ci(level = 0.95)
```

#### There is no evidence that those who sleep 10+ hours per day are more likely to strength train every day of the week.An alternative hypothesis can display evidence that those who sleep 10+ hours per day are more likely to strength train every day of the week.
...
#### I can say that, there is a 95% confident level that the true proportion of those who sleep 10+ hours daily, that also strength train every day of the week it rages between 22.12% and 32.05%.
...
#### Personally, I will select a sample of 0.10 or less than 10% of the local population.
