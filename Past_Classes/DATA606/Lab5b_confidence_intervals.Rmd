...
**It will be 0.62 or 62% of the adults**

1.  Would you expect another student's sample proportion to be identical to yours? 
    Would you expect it to be similar? Why or why not?

**No because it is almost certain that another student's proportion might have a different outcome that it might be similar but not exactly the same.**
...
**95% confidence means that even if we don't know what the full population looks like, we are a 95% confident about the true proportion of adults who think about the impact of climate change in their communities, based on the sample proportions.**
...
**No, it does not capture the true population proportion of US adults at the same value.I am working by myself, however, I think that I would have a closer value with a classmate than with the true population.**
...
**I'll expect most students to capture a true population mean between 0,5 and 1, because anything above that it might be consider unusual.**
...
**The intervals included in the true population proportion are 57/60, which is exactly equal to the confidence level**
**C:/Users/vitug/OneDrive/Desktop/DATA_606/plot1**
* * *
...
**Changing the confidence level greater than 95%, the bounds would be narrower, and therefore capturing the true population proportion. On the other hand, if a change the confidence level less the 95%, the level will become wider.**

1.  Using code from the **infer** package and data front the one sample you have 
...
**I've decided to set a confidence level to 90%, with this values, the true mean lies between 45% and 67% of US adults who believe climate change affects their community.**
...
**The results from the app are higher than the selected confidence intervals.**
```{r}
ggplot(data = samp, aes(x = climate_change_affects)) +
  geom_bar() +
  labs(
    x = "Intervals",
...
**I changed a CL to 66%, the width of the interval is narrower which is what I was expecting from it.**
...
**After experimenting with several sample sizes, I've realized that the higher the confidence level, the wider the intervals become. As the confidence interval gets smaller, the intervals get wider.**
...
**It does not affect the standard error and the width it becomes wider.**
